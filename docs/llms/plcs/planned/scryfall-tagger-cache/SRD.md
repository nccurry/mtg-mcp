...
background refresh, category mapping, heuristic tags, image tagging, and live
Scryfall lookup are out of scope.
...
| TAG-003 | Must | Each completed card snapshot shall preserve Oracle ID, printing used, retrieval time, source URL, acquisition version, checksum, and status. | SQLite round-trip tests pass. |
| TAG-004 | Must | Assignments shall preserve Tagger tag ID/slug/name/type, direct/ancestor relation, accepted/rejected state, and raw extension data. | Rich GraphQL fixture round trips. |
...
| TAG-008 | Must | Acquisition shall bootstrap CSRF/session state from HTML and POST the observed GraphQL query with the same cookie session. | Fake HTML/cookie/GraphQL tests pass. |
...
| Fidelity | Direct/inherited/type/status/raw association evidence preserved. |
