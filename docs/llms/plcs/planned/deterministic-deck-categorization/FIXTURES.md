...
Given a revisioned local Commander deck, an installed Scryfall corpus, and
caller-authored category rules, validate and preview exact multi-category
assignments with direct/inherited evidence. Apply only the unchanged preview,
then verify revision, assignments, primary state, preserved unrelated
categories, and backup/export behavior. No tool invents or recommends a rule.
