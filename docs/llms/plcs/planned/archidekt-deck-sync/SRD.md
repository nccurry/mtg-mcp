...
| ARCH-011 | Must | Remote delete shall require exact deck ID, fresh fingerprint, and explicit confirmation. A verified delete contract is mandatory; if it is unavailable or drifted, this child remains incomplete and cutover is blocked rather than emulating deletion. | Delete fixture passes and the unsupported case fails the completion gate. |
| ARCH-012 | Must | Requests shall be serialized at no faster than one per second with a maximum of 150 provider requests per tool call. The cap is a client safety ceiling sized for a 100-card Commander deck plus bounded metadata/category and verification requests, not a provider guarantee. A plan predicted to exceed it shall fail before the first mutation. | Fake-clock, preflight, zero-write, and cap tests pass. |
| ARCH-013 | Must | 403/429 shall stop immediately; mutation requests shall not be retried automatically after ambiguous transport/5xx failure. | Request-count and partial-status tests pass. |
...
| ARCH-017 | Must | Real mutations shall occur only in explicit `Category=Live` throwaway-deck tests with cleanup. | Live test discovery and safety guards pass. |
...
| Provider safety | One request/second, hard cap, stop-on-block, no ambiguous write retries. |
