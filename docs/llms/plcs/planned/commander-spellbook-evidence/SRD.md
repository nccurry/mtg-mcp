...
| CSB-CASE-003 | An LLM has a saved deck revision. | It receives the source's included and almost-included groups without a recommendation. |
...
| CSB-002 | Must | `spellbook_variant_search` must send a non-empty source query without local parsing or rewriting. | A fake request proves a query with quotes, whitespace, and parentheses is encoded once without changing its source characters, and only one bounded provider request occurs. |
...
| CSB-004 | Must | `spellbook_deck_combos_find` must read one saved deck revision and send only its `main` and `commander` entries. | Tests show revision conflicts stop before HTTP and skipped zones appear in the result. |
| CSB-005 | Must | The tools must preserve Commander Spellbook's response fields and group names. | Fixture tests retain unknown fields and groups such as `included` and `almostIncluded`. |
| CSB-006 | Must | Every successful result must name the source, endpoint, retrieval time, cache state, checksum, source credit URL, and limitations. | Tool and serialization tests assert the evidence envelope. |
...
| CSB-011 | Must | The cache must store only a request hash, response data, checksums, and times. | Database tests show no raw deck request or raw query text is written, and a changed contract checksum does not reuse an old response. |
...
| CSB-013 | Must | Provider errors must become typed safe results. | Tests cover 400, 404, 429, 5xx, network errors, invalid JSON, large responses, and cancellation. |
...
| --- | --- | --- | --- |
| `spellbook_variant_search` | `sourceQuery` | `limit`, `offset`, `groupByCombo` | Query: 1–512 characters. Limit: 1–25. Offset: 0–1,000. |
| `spellbook_variant_get` | `variantId` | None | ID: 1–256 characters. |
| `spellbook_deck_combos_find` | `deckId`, `expectedRevision` | `sourceQuery`, `limit`, `offset`, `groupByCombo` | The same page bounds. Source input: at most 600 main rows and 12 commander rows. |
...
| Privacy | A deck lookup uses the local cache. | The stored cache key is a one-way hash, not the card list or query text. |
...
| CSB-006 | [Evidence envelope](SADD.md#evidence-envelope) | Serialization and MCP tool tests |
