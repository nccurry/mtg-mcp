...
The adapter returns a provider-shaped JSON value inside a small evidence record.
...
| `SpellbookContract` | The observed API version, contract checksum, limits, and credit URL. | BCL. | Contract tests. |
| `SpellbookEvidence` | A detached source response with evidence metadata. | Core result type and JSON. | Serialization tests. |
...
The tool does not send a blank query. This prevents an accidental broad browse
...
6. Reject an empty selection, more than 12 commander rows, or more than 600
   main rows before HTTP.
...
`SpellbookEvidence` keeps the provider response as a detached `JsonElement`.
...
| `limitations` | Short source and local limits that affect interpretation. |
| `data` | The lossless provider JSON. |

The App adds a local-deck selection record to a deck lookup result. That record
contains the deck ID, revision, entries sent to the source, and skipped entries.
It is not stored in the adapter cache.
...
| --- | --- | --- |
| `response_cache` | Operation, request hash, response JSON, response checksum, source API version, contract checksum, and retrieval time. | Raw query text, raw deck request, user account data, or usage history. |
| `request_pacing` | Next reserved request start and cooldown end. | Request content or source response. |

The adapter builds a canonical request value in memory. It contains the
...
A cache hit requires the current contract checksum. If the checked-in source
contract changes, the old row is a miss and is removed during normal cleanup.
...
```text
MtgMcp.App/Spellbook --> MtgMcp.Decks
                         MtgMcp.Spellbook --> MtgMcp.Core

MtgMcp.Core <-- MtgMcp.Spellbook
```
...
| Transport tests | Headers, fixed origin, response bound, statuses, no retry, and redaction. |
| Service tests | Query bounds, ID validation, JSON preservation, and typed result mapping. |
