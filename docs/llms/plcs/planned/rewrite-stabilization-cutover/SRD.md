...
| CUT-003 | Must | The stable MCP surface shall match the canonical manifest derived from the completed, approved capability children, expose one capability resource, and expose zero prompts. The current planning baseline is 71 tools, but no count is preserved solely for compatibility. | Approved schema snapshot matches the derived manifest byte-for-byte after canonicalization. |
| CUT-004 | Must | Per-mode visibility shall match every approved tool's operation-mode requirement in that manifest. The current planning baseline is 44 tools in `read-only`, 66 in `local`, and 71 in `remote`; approved design changes recalculate those totals. | Per-mode discovery fixtures and manifest reconciliation pass. |
