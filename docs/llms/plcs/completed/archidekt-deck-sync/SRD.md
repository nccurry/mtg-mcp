...
| ARCH-012 | Must | Requests shall be serialized process-wide per configured account with at least two seconds between starts and no more than 30 starts in any rolling 60 seconds. Each tool call shall also have one shared maximum of 150 provider requests across authentication, preview replay, apply, and verification. The bounds are client safety ceilings, not provider guarantees; a predicted call above the remaining cap shall fail before the first mutation. | Fake-clock, multi-instance, composed-scope, rolling-window, preflight, zero-write, and cap tests pass. |
...
| ARCH-020 | Must | Folder list/get shall return a fresh provider tree or folder detail with exact folder ID, name, visibility when present, parent ID, path, child-folder IDs, deck IDs/summaries, retrieval metadata, source checksum, and preserved unknown extension fields. Because the observed tree omits deck rows, the adapter shall join a fresh authenticated owned-deck listing rather than claim the tree is complete. | Sanitized recursive-tree, direct-root, joined-deck, empty, private, and unknown-field fixtures pass. |
...
| ARCH-025 | Must | Snapshot create/update/delete shall use exact deck/snapshot IDs, return verified provider state, and require fresh snapshot identity plus explicit confirmation for delete. The observed update contract supports renaming only; description is accepted only during creation. | Request/response, stale identity, rename, delete, and absence fixtures pass. |
| ARCH-026 | Must | Snapshot restore shall use separate preview/apply tools. Preview shall diff a freshly fetched snapshot against the fresh current remote deck. Apply shall verify snapshot checksum, restorable-content fingerprint, remote deck fingerprint, and preview fingerprint, then use the same guarded primitive deck-write planner as push. Current deck name, visibility, and folder placement shall be preserved because the observed snapshot does not own those fields. Final verification shall require restorable content equality while reporting provider relation IDs that were necessarily regenerated. | Exact restore, provider-ID churn, stale source/target, account-metadata preservation, partial write, and final content-fingerprint fixtures pass. |
...
- [x] Essential read and sync workflows pass fixtures.
- [x] Conflict and partial-write behavior is proven.
- [x] At least one discoverable live throwaway workflow is documented and opt-in.
- [x] Verified remote deletion leaves no throwaway deck after the live workflow.
- [x] Folder and named-snapshot lifecycle/restore pass offline fixtures and the disposable live workflow.
- [x] Automatic history, package, tag, collaboration, social, and account-administration features are absent from the surface.
