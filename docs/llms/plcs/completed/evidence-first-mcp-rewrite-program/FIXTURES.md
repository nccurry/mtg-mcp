...
| --- | ---: | ---: | ---: | --- |
| Current implemented `default` | 32 | 54 | 54 | Default-enabled `decks`, `scryfall`, and eight read-only `stats_*` tools, including the two categorization reads. |
| Current implemented `all` | 57 | 80 | 93 | Adds the opt-in implemented `archidekt` and `playgroup` toolsets. |
