...
/// <summary>
/// Finds bounded Commander candidates by combining catalog search with source-backed deck counts.
/// </summary>
public sealed class DeckCommanderCandidateSearchService
{
    /// <summary>
    /// Searches and resolves Commander card candidates.
    /// </summary>
    private readonly ICardCatalog cardCatalog;

    /// <summary>
    /// Supplies source-backed aggregate rows for each candidate commander.
    /// </summary>
    private readonly DeckCommanderEvidenceService commanderEvidence;

    /// <summary>
    /// Creates a Commander candidate search collaborator with explicit catalog and evidence dependencies.
    /// </summary>
    public DeckCommanderCandidateSearchService(
        ICardCatalog cardCatalog,
        DeckCommanderEvidenceService commanderEvidence)
    {
        this.cardCatalog = cardCatalog;
        this.commanderEvidence = commanderEvidence;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (CardSearchResult candidate in candidates)
        {
            ...
            try
            {
                aggregate = await commanderEvidence.GetCommanderAggregateCardsAsync(
                        card.Name,
                        theme: null,
                        source: "edhrec",
                        limit: 1,
