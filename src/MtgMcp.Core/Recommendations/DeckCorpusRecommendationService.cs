...
/// <summary>
/// Builds corpus-backed recommendation, evidence search, and source status results for saved decks.
/// </summary>
public sealed class DeckCorpusRecommendationService
{
    /// <summary>
    /// Loads saved deck workspaces for corpus filtering.
    /// </summary>
    private readonly IDeckWorkspaceRepository repository;

    /// <summary>
    /// Resolves candidate card metadata and Scryfall presentation links.
    /// </summary>
    private readonly ICardCatalog cardCatalog;

    /// <summary>
    /// Builds budget replacement plans before corpus enrichment is attached.
    /// </summary>
    private readonly DeckReplacementService replacements;

    /// <summary>
    /// Supplies normalized card evidence, exemplar decks, and discussions.
    /// </summary>
    ...
    /// <summary>
    /// Creates a corpus recommendation collaborator with explicit storage, catalog, replacement, and source dependencies.
    /// </summary>
    public DeckCorpusRecommendationService(
        IDeckWorkspaceRepository repository,
        ICardCatalog cardCatalog,
        DeckReplacementService replacements,
    ...
    {
        RecommendationAnalysisBudget budget = RecommendationAnalysisBudget.FromDepth(analysisDepth);
        RecommendationPlanResult planResult = await replacements
        ...
        foreach (ReplacementSuggestion suggestion in planResult.Suggestions.Take(budget.MaxRecommendations))
        {
            ...
            recommendations.Add(new CorpusRecommendation
            {
                ...
                Tags = card is null ? [] : DeckRoleClassifier.Classify(DeckRecommendationCardFacts.CreateCandidateCard(card)).Tags,
        ...
        CancellationToken cancellationToken)
    {
        ...
        string? commander = DeckServiceHelpers.FindCommanderName(workspace, intent);
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (CardInfo card in cards.Values)
        {
            if ((!includeExistingCards && existing.Contains(card.Name))
                ...
                || !IsPriceWithinBudget(DeckRecommendationCardFacts.ReadUsdPrice(card), maxPrice)
