...
public sealed partial class ArchidektGateway
{
    public Task<AuthStatus> GetAuthStatusAsync(CancellationToken cancellationToken)
    {
        ...
        AuthStatus status = new()
        {
            ...
            HasEmailPassword = !string.IsNullOrWhiteSpace(loaded.Email) && !string.IsNullOrWhiteSpace(loaded.Password),
    ...
    private ArchidektCredentials LoadCredentials()
    {
        ...
        ArchidektCredentials loaded = new()
        {
            ...
            UserId = FirstNonEmpty(options.UserId, Environment.GetEnvironmentVariable("ARCHIDEKT_USER_ID")),
            Email = FirstNonEmpty(options.Email, Environment.GetEnvironmentVariable("ARCHIDEKT_EMAIL")),
    ...
    private async Task EnsureAuthenticatedAsync(bool required, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (options.EnableUsernamePasswordLogin
                && (!string.IsNullOrWhiteSpace(loaded.Email) || !string.IsNullOrWhiteSpace(loaded.Username))
                && !string.IsNullOrWhiteSpace(loaded.Password))
            {
                string loginIdentifier = FirstNonEmpty(loaded.Email, loaded.Username)!;
                string? jwt = await TryLoginAsync(loginIdentifier, loaded.Password, cancellationToken).ConfigureAwait(false);
    ...
    private async Task<string?> TryLoginAsync(string usernameOrEmail, string password, CancellationToken cancellationToken)
    {
    ...
    private static ArchidektCredentials LoadCredentialsFile(string credentialsFile)
    {
        ...
        {
            throw new InvalidDataException($"Archidekt credentials file '{credentialsFile}' could not be read: {exception.Message}", exception);
        }
        ...
        if (trimmed.StartsWith('{'))
        {
            try
            {
                return JsonSerializer.Deserialize<ArchidektCredentials>(text, SerializerOptions) ?? new ArchidektCredentials();
            }
            catch (JsonException exception)
            {
                throw new InvalidDataException(
                    $"Archidekt credentials file '{credentialsFile}' looks like JSON but could not be parsed. JSON requires double quotes around keys and string values; escape backslashes as \\\\ and double quotes as \\\". To avoid JSON escaping, use key=value lines instead.",
                    exception);
            }
        }
    ...
    private static ArchidektCredentials ParseKeyValueCredentials(string credentialsFile, string text)
    {
        ...
        while (reader.ReadLine() is { } line)
        {
            ...
            {
                throw new InvalidDataException(
                    $"Archidekt credentials file '{credentialsFile}' is not valid JSON or key=value format. Line {lineNumber} must look like username=value, password=value, jwt=value, accessToken=value, or refreshToken=value.");
            }
    ...
    private static void ApplyCredentialValue(ArchidektCredentials credentials, string key, string value)
    {
        ...
        else if (normalized.Equals("accesstoken", StringComparison.OrdinalIgnoreCase)
        ...
        else if (normalized.Equals("refreshtoken", StringComparison.OrdinalIgnoreCase)
            || normalized.Equals("refresh", StringComparison.OrdinalIgnoreCase))
        {
            credentials.RefreshToken = value;
        }
        else if (normalized.Equals("userid", StringComparison.OrdinalIgnoreCase)
