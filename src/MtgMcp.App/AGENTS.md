...
- Treat annotations as accurate risk hints, not authorization. Enforce write
  capability with `OperationModeGuard` and keep apply-only tools hidden outside
  apply mode.

## Host Boundary

- Keep transport, DI, configuration, logging, CLI, MCP error mapping, and
  provider composition in App. Keep domain decisions in Core and provider
  contracts in adapters.
