...
public sealed class MtgResources
{
    ...
    public async Task<string> GetDeckAsync(string deckId, CancellationToken cancellationToken = default)
    {
        DeckWorkspace workspace = await decks.GetDeckResourceAsync(deckId, cancellationToken).ConfigureAwait(false);
    ...
    public async Task<string> GetDeckSummaryAsync(string deckId, CancellationToken cancellationToken = default)
    {
        object summary = await decks.GetDeckSummaryAsync(deckId, cancellationToken).ConfigureAwait(false);
        return JsonSerializer.Serialize(summary, JsonOptions);
    }

    [McpServerResource(UriTemplate = "mtg://scryfall/syntax-cheatsheet", Name = "Scryfall Syntax Cheatsheet")]
    ...
    [McpServerResource(UriTemplate = "mtg://formats/{format}/deck-rules", Name = "Format Deck Rules")]
    [Description("Lightweight deck construction rules for common formats.")]
    public string GetFormatRules(string format)
    {
        return format.Trim().ToLowerInvariant() switch
        {
            "commander" or "edh" => "Commander: 100 included cards, singleton except basic lands, one commander or legal commander pair, color identity restrictions.",
            "standard" => "Standard: at least 60 mainboard cards, optional sideboard up to 15, up to four copies except basic lands.",
            "modern" => "Modern: at least 60 mainboard cards, optional sideboard up to 15, up to four copies except basic lands.",
            "legacy" => "Legacy: at least 60 mainboard cards, optional sideboard up to 15, up to four copies except basic lands unless restricted or banned.",
            "pauper" => "Pauper: at least 60 mainboard cards, optional sideboard up to 15, only cards printed at common in a supported release.",
            _ => "Generic constructed: at least 60 mainboard cards, optional sideboard up to 15, up to four copies except basic lands."
        };
    }

    [McpServerResource(UriTemplate = "mtg://usage/workspace-selection", Name = "Workspace Selection Guidance")]
    ...
    public string GetWorkspaceSelectionGuidance()
    {
        ...
        Use Archidekt mode when the user provides an Archidekt deck id or URL, asks to update an online deck, or asks for Archidekt checkpoints.
        If local and Archidekt are both plausible, ask the user which workspace mode to use before creating or opening a workspace.
        Never enable Archidekt writeback unless the user explicitly asks to update, organize, tag, move cards, checkpoint, or otherwise persist changes to Archidekt.
        If Archidekt writeback intent is unclear, ask whether edits should write back to Archidekt or stay local-only.
        Prefer start_deck_workspace as the first deck workspace tool because it requires an explicit mode and writeback choice.
        """;
    }

    [McpServerResource(UriTemplate = "mtg://usage/operation-modes", Name = "Operation Mode Guidance")]
    ...
    public string GetEffectiveConfiguration()
    {
        ...
        {
            if (pair.Value is not null && pair.Key.StartsWith("MtgMcp", StringComparison.OrdinalIgnoreCase))
            {
    ...
    public async Task<string> GetArchidektAuthStatusAsync(CancellationToken cancellationToken = default)
    {
        AuthStatus status = await archidektGateway.GetAuthStatusAsync(cancellationToken).ConfigureAwait(false);
