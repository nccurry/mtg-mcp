...
public sealed class OnCurveRequestValidatorTests
{
    ...
    public static TheoryData<object?> InvalidRequests => new()
    {
        ...
        ValidRequest() with { Mainboard = Entries(OnCurveRequestValidator.MaximumMainboardEntries + 1) },
        ValidRequest() with { CandidateLandEntryIds = null },
        ValidRequest() with { LandRules = null },
        ValidRequest() with { TurnLimit = OnCurveRequestValidator.MinimumTurnLimit - 1 },
        ValidRequest() with { TurnLimit = OnCurveRequestValidator.MaximumTurnLimit + 1 },
        ValidRequest() with { SampleCount = OnCurveRequestValidator.MinimumSampleCount - 1 },
        ValidRequest() with { SampleCount = OnCurveRequestValidator.MaximumSampleCount + 1 },
